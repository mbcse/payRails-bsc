...
const { UNISWAP_V3_BASED_FACTORY_ABI, ALGEBRA_BASED_FACTORY_ABI, ALGEBRA_BASED_POOL_ABI } = require("./factoryABI");
const redis = require("../DataLayer/redis/redis");

const getUniswapFactoryAddress = (chainId) => {
    switch(chainId) {
        case 8453: return "0x33128a8fC17869897dcE68Ed026d694621f6FDfD";
...
const getUniswapV3PathFee = async (token1Address, token2Address, provider) => {
    ...
    if(await uniswapV3PoolExists(token1Address, token2Address, provider, '3000')){
        console.log("Pool Found with fee 0.3%")
        return '3000'
    }else if(await uniswapV3PoolExists(token1Address, token2Address, provider, '500')){
        console.log("Pool Found with fee 0.05%")
        return '500'
    }else if(await uniswapV3PoolExists(token1Address, token2Address, provider, '10000')){
        console.log("Pool Found with fee 1%")
        return '10000'
    }else if(await uniswapV3PoolExists(token1Address, token2Address, provider, '100')){
...
const uniswapV3PoolExists = async (token1Address, token2Address, provider, fee) => {
    console.log(`Finding Pool with fee: ${fee/10000}% between ${token1Address} <-> ${token2Address}`)
    const {chainId} = await provider.getNetwork();
    const factoryAddress = getUniswapFactoryAddress(chainId)
    try{
        ...
        if(chainId == 1088){
            ...
            const factoryContract = new ethers.Contract(factoryAddress, UNISWAP_V3_BASED_FACTORY_ABI, provider);
            const poolAddress = await factoryContract.getPool(token1Address, token2Address, fee);
            console.log("PoolAddress => "+ poolAddress)
            if(poolAddress === '0x0000000000000000000000000000000000000000') {
                console.log(`No Pool Found with fee tier: ${fee/10000}%`)
                return false
...
const getEncodedUniswapV3Path = async (path, provider) => {
    ...
    for(let i=0; i<pathLen; i++){
        ...
        if(i+1 != pathLen) {
            const fee = await getUniswapV3PathFee(path[i], path[i+1], provider)
            console.log(`Using Fee Tier -> ${fee/10000} %`)
            // typeArray.push('uint16')
            // pathArray.push(fee)
        }

    }
    console.log(`Final Uniswap V3 Path Constructed = ${pathArray}`)
...
module.exports = {getPath, getEncodedUniswapV3Path, savePathDataInRedis}